...
export default function Peg({ peg, index, capacity, selected, onClick }: PegProps) {
  ...
  return (
    <button
      ...
      className={`group relative flex flex-col items-center justify-end rounded-2xl border-2 px-2 pt-2 pb-1 sm:px-3 md:px-4 md:pt-3 select-none transition ${
        selected
    ...
    >
      <div className="absolute top-2 bottom-6 left-1/2 -translate-x-1/2 w-1.5 md:w-2 rounded-full bg-slate-300 dark:bg-slate-700" />
      <div
      ...
      <div className="mt-1 h-2 md:h-2.5 w-full rounded-full bg-slate-400 dark:bg-slate-600" />
    </button>
  );
}
